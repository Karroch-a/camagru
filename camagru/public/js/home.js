function like(image_n, id, id_user)
			{
				if (id_user !== '')
				{
			...
			function cmnt(image_n, i, j, username, id_user)
			{
				...
				xhttp.onreadystatechange = function() {
					...
					if (document.getElementById(i).value !== '')
					{
						...
						icon.setAttribute("class", "fa fa-trash fa-fw");
			...
			function deletcmnt(id, id_user, cmnt_id)
			{
				var xhttp = new XMLHttpRequest();
				xhttp.open("POST", window.location.href, true);
				picture = 'id_cmnt=' + id + '&id_user=' + id_user;
				xhttp.onreadystatechange = function() {
					if (this.readyState == 4 && this.status == 200) {
						res = JSON.parse(xhttp.responseText);
						if (res)
						{
							var myobj = document.getElementById(cmnt_id);
							myobj.remove();
						}
						}
					};
